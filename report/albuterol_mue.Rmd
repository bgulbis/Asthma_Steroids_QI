...
```{r, fig.cap="Duration of steroid use. Methylprednisolone, prednisolone, and prednisone are combined and the total duration is displayed. Patients who received both dexamethsone and one of the other steroids are excluded, as are patients who only received one dose of any steroid."}
...
data_albut_total %>%
    ...
    knitr::kable(
        digits = 1,
        caption = "Cumulative albuterol dose (in mg) from time of continuous initiation until scheduled q4 hour dosing initiated for 2016 compared with 2017",
        col.names = c(
            "Year",
            "Mean",
            "SD",
...
data_albut_total %>%
    left_join(
        data_demographics[c("millennium.id", "weight")],
        by = "millennium.id"
    ) %>%
    mutate(cum.dose.kg = cum.dose / weight) %>%
    group_by(year) %>%
    summarize_at(
        "cum.dose.kg",
        funs(
    ...
    knitr::kable(
        digits = 1,
        caption = "Cumulative albuterol dose (in mg/kg) from time of continuous initiation until scheduled q4 hour dosing initiated for 2016 compared with 2017",
        col.names = c(
            "Year",
            "Mean",
            "SD",
