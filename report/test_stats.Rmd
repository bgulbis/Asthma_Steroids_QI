...
```{r}
p_cat2 <- function(df, y, ..., subgrp = NULL, exact = FALSE) {
    ...
    y_val <- dplyr::pull(df, y_var)

    df %>%
        select(!!!x) %>%
        map(~ run_test(.x, y_val) %>% broom::tidy())
}

test_fx <- function(df) {
    wilcox.test(age ~ year, data = df)
}

test_fx_mult <- function(df, x, y) {
    x_var <- rlang::quo_name(rlang::enexpr(x))
    y_var <- rlang::quo_name(rlang::enexpr(y))

    f <- rlang::parse_expr(paste0(x_var, " ~ ", y_var))

    wilcox.test(formula = as.formula(f), data = df, conf.int = TRUE)
    # 
    # x_var <- dplyr::pull(df, !!rlang::enexpr(x))
    # y_var <- dplyr::pull(df, !!rlang::enexpr(y))
    # 
    # wilcox.test(x_var, y_var)
}


fx <- function(df, y, ..., normal = FALSE) {
    # y_var <- rlang::quo_name(rlang::enexpr(y))
    y_var <- dplyr::pull(df, !!rlang::enexpr(y))
    ...
    df %>%
        select(!!!x_var) %>%
        map(~ run_test(.x, y = y_var, conf.int = TRUE) %>% broom::tidy()) 
        # map(run_test, y = y_var, conf.int = TRUE) 
        # map(broom::tidy) 
        # flatten_dfr()
        # unnest(.id = "var_name")
        # tidyr::gather("var_name", "value", !!!x) %>%
        # dplyr::group_by(!!!var_name) 
        # map(run_test, formula = !!f, conf.int = TRUE) 
    # run_test(formula == !!f, data = df, conf.int = TRUE)
}
...
f <- function(df, y, ...) {
    ...
    df_sum <- df %>%
        ...
        summarize_at(
            vars(!!!x_var), 
            funs(mean, sd, median, IQR), 
            na.rm = TRUE
        ) 
    
    if (length(x_var) > 1) {
        df_sum <- df_sum %>%
            gather("var.name", "result", -!!y_var) %>%
            separate("var.name", c("var.name", "measure"), sep = "_") %>%
            spread("measure", "result")
    }
    ...
    #         "IQR"
    #     ) %>%
    #     spread(!!y_var, "all.measures", sep = "_")
    # separate()
...
y <- x %>%
    mutate(
        sum = map(data, f, y = year, age, length.stay),
        # model2 = map(data, test_fx),
        model = map(data, test_fx_mult, x = age, y = year),
        # model2 = map(data, test_fx_mult, x= length.stay, y = year),
        # model = map(data, fx, y = year, age, length.stay),
        tidy = map(model, broom::tidy)
        # tidy2 = map(model2, broom::tidy)
    ) %>%
    unnest(tidy)
    # mutate_at(c("model", "model2"), map, .f = broom::tidy)
    
    
y$sum[[1]] %>%
    separate(2, c("iqr1", "med1"), sep = "_", fill = "left", convert = TRUE) %>%
    separate(4, c("iqr2", "med2"), sep = "_", fill = "left", convert = TRUE)

y %>%
    unnest(sum, tidy, .drop = TRUE) 
...
f3 <- function(x, df, y) {
    df %>%
        ...
        mutate(!!"var_name" := rlang::quo_name(x))
}
...
f2 <- function(df, y, ...) {
    ...
    map_df(x_var, f3, df = df, y = y_var)
}

y <- x %>%
    mutate(
        sum = map(data, f2, y = year, age, length.stay)
    ) %>%
    unnest(sum)
