...
```{r, eval=FALSE}
# mag hypotension
=IF(AND([@[Age Category]]=1,[@[vital.result]]<60),1,IF(AND([@[Age Category]]=2,[@[vital.result]]<70),1,IF(AND([@[Age Category]]=3,[@[vital.result]]<(70+(2*[@[Age in years]]))),1,IF(AND([@[Age Category]]=4,[@[vital.result]]<90),1,0))))

# age category
=IF([@[age.days]]<=28,1,IF(AND([@[age.days]]>28,[@[age.days]]<=365),2,IF(AND([@[age.days]]>365,[@[age.days]]<=3649),3,IF([@[age.days]]>3649,4,"N/A"))))


# wrong dose
=IF(AND([@[Weight>40 kg]]=1,[@[med.dose]]<>2000),1,0)

```

```{r}
tmp <- data_demographics %>%
    mutate(
        age.cat = case_when(
            age.days < 28 ~ 1,
            age.days <= 365 ~ 2,
